# Exercise 1: Blue-Green Deployment - Part 2 (⭐ Foundation)
...
### Step 7: Create Green Deployment Configuration
...
**Expected Output:**
Create `kubernetes/green-deployment.yaml`:
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
**Expected Output:**
Create `monitoring/deployment_monitor.py`:
```python
import asyncio
import time
...
**Expected Output:**
Create `tests/test_blue_green_deployment.py`:
```python
import pytest
import asyncio
import httpx
...
1. ✅ Built a complete blue-green deployment system
...
- **Easy Rollback**: Previous version remains available for quick rollback
- **Testing in Production**: New version can be fully tested with production data
...
2. **Skipping health checks** - Never switch without verifying new environment health
3. **Insufficient monitoring** - Monitor both environments during and after switch
4. **Resource doubling** - Plan for 2x resource usage during deployment
...
Extend the deployment to work across multiple Azure regions with Traffic Manager.

### Challenge 2: Automated Rollback
...
Implement auto-scaling that reduces resources in the inactive environment.

## 📚 Additional Resources

- [Azure Blue-Green Deployments](https://learn.microsoft.com/azure/architecture/guide/azure-devops/blue-green-deployment)
- [Kubernetes Blue-Green Tutorial](https://kubernetes.io/blog/2018/04/30/zero-downtime-deployment-kubernetes/)
