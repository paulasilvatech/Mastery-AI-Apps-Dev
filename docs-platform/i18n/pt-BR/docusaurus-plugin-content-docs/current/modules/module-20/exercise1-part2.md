...
# Exercício 1: Blue-Green implantação - Partee 2 (⭐ Foundation)
...
### Step 7: Create Green implantação Configuration
...
**Expected Output:**
Create `kubernetes/green-implantação.yaml`:
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
## 🚀 Partee 4: implantação Execution

### Step 8: Create Monitoring Painel
...
**Expected Output:**
Create `monitoring/implantação_monitor.py`:
```python
import asyncio
import time
...
**Expected Output:**
Create `tests/test_blue_green_implantação.py`:
```python
import pytest
import asyncio
import httpx
...
1. ✅ Built a complete blue-green implantação system
2. ✅ Implemented health checks and monitoring
3. ✅ Created automated implantação scripts
...
- **Testing in Production**: New version can be fully tested with produção data
...
4. **Resource doubling** - Plan for 2x resource usage during implantação
...
Extend the implantação to work across multiple Azure regions with Traffic Manager.

### Challenge 2: Automated RollVoltar
...
- [Azure Blue-Green Deployments](https://learn.microsoft.com/azure/architecture/guide/azure-devops/blue-green-implantação)
- [Kubernetes Blue-Green Tutorial](https://kubernetes.io/blog/2018/04/30/zero-downtime-implantação-kubernetes/)
